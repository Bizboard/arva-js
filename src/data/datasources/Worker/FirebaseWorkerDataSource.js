...
@provide(DataSource)
export class FirebaseWorkerDataSource extends DataSource {
     ...
     **/
    constructor(path, options = {orderBy: '.priority'}, firebaseSettings = {}) {
        super(path);

        this._path = path;
        this._root = firebaseSettings.databaseURL || '';
        ...
        this._dataReference = new WorkerClient(path, options, firebaseSettings.databaseURL ? firebaseSettings : null);
        this.handlers = {};
    ...
    async build() {
        ...
        if (this._dataReference.orderBy && this.options.orderBy === '.priority') {
            await this._dataReference.orderByPriority();
        } else if (this.options.orderBy && this.options.orderBy === '.value') {
            await this._dataReference.orderByValue();
        } else if (this.options.orderBy && this.options.orderBy !== '') {
            await this._dataReference.orderByChild(this.options.orderBy);
        }

        if (this.options.limitToFirst !== undefined) {
          await this._dataReference.limitToFirst(this.options.limitToFirst);
        } else if (this.options.limitToLast !== undefined) {
          await this._dataReference.limitToLast(this.options.limitToLast);
        }
    }
     ...
     */
    toString() {
        return (this._root + this._path).toString();
    }
    ...
    child(childName, options = {}) {
        return new FirebaseWorkerDataSource(`${this.path()}/${childName}`, options);
    }
     ...
     */
    key() {
        return (this._path.split('/')[this._path.split('/').length - 1] || '').toString();
    }
     ...
     * @returns {Promise} A new FirebaseDataSource pointing to the injected data.
     */
    push(newData) {
        return new Promise((resolve)=> {
            this._dataReference.push(newData).then((pushResult)=> {
                resolve(new FirebaseWorkerDataSource(`${this.path()}/${pushResult.key}`, {synced: Promise.resolve()}))
            });
        });
    }
    ...
    orderByChild(childKey) {
        return new FirebaseWorkerDataSource(this.path(), _.merge({}, this.options, {orderBy: childKey}));
    }
     ...
     */
    orderByKey() {
        return new FirebaseWorkerDataSource(this.path(), _.merge({}, this.options, {orderBy: '.key'}));
    }
     ...
     */
    orderByValue() {
        return new FirebaseWorkerDataSource(this.path(), _.merge({}, this.options, {orderBy: '.value'}));
    }
     ...
     */
    limitToFirst(amount) {
        return new FirebaseWorkerDataSource(this.path(), _.merge({}, this.options, {limitToFirst: amount}));
    }
     ...
     */
    limitToLast(amount) {
        return new FirebaseWorkerDataSource(this.path(), _.merge({}, this.options, {limitToLast: amount}));
    }
    ...
    on(event, handler, context = this) {
        let callback = (data) => {
            let newChildSnapshot = new FirebaseSnapShot(data, this);
            handler.call(this, newChildSnapshot, data.prevSiblingId || undefined);
        };

        let boundHandler = this.handlers[handler] = callback.bind(this);
