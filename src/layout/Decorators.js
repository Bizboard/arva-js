...
let createChainableDecorator = function (method, type) {
  ...
  let methodToReturn = function (viewOrRenderable, renderableName, descriptor) {
    if (methodToReturn.lastResult) {
      let queuedDecorators = getPreviousResults(methodToReturn.lastResult)
      for (let queuedDecorator of queuedDecorators) {
        queuedDecorator(viewOrRenderable, renderableName, descriptor)
      }
    }
    ...
    return method(decorations, type)
  }
  let root = this
  if (this.originalObject) {
    methodToReturn.lastResult = lastResult
    root = methodToReturn.originalObject = lastResult.originalObject
  } else {
...
class Layout {
   ...
   */
  renderable = () => {
    return this.createChainableDecorator(() => {}, decoratorTypes.childDecorator)
  }
...
export const event = {
  ...
  pipe: function (pipeToName) {
    return function pipe (view, renderableName, descriptor) {
      let renderable = prepDecoratedRenderable(view, renderableName, descriptor, pipe)
